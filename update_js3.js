...
// 1. Update initMap to store user location silently
newJs = newJs.replace(
  /if \(navigator\.geolocation\) \{\s*navigator\.geolocation\.getCurrentPosition\(\s*\(pos\) => map\.flyTo\(\[pos\.coords\.latitude, pos\.coords\.longitude\], 13, \{ duration: 1\.5 \}\),\s*\(\) => \{\},\s*\{ timeout: 8000, maximumAge: 60000 \}\s*\);\s*\}/,
  "if (navigator.geolocation) {\n        navigator.geolocation.getCurrentPosition(\n          (pos) => {\n            userOrigin = { lat: pos.coords.latitude, lng: pos.coords.longitude, label: 'My Location' };\n          },\n          () => {},\n          { timeout: 8000, maximumAge: 60000 }\n        );\n      }"
);

// 2. Add sidebar mode config and functions before "Start App"
...
// Insert sidebar functions before "Start App"
newJs = newJs.replace(
  '    // Start App',
  sidebarFunctions + '    // Start App'
);

// 3. Remove old functions
// Remove setupAutocomplete
newJs = newJs.replace(
  /    function setupAutocomplete\(inputId, suggestId\) \{[\s\S]*?\n    \}/,
  ''
);

// Remove selectSearchResult
newJs = newJs.replace(
  /    async function selectSearchResult\(inputId, suggestId, item\) \{[\s\S]*?\n    \}/,
  ''
);

// Remove swapOriginDest
newJs = newJs.replace(
  /    function swapOriginDest\(\) \{[\s\S]*?\n    \}/,
  ''
);

// Remove startVoice
newJs = newJs.replace(
  /    function startVoice\(inputId\) \{[\s\S]*?\n    \}/,
  ''
);

// Remove armPicker
newJs = newJs.replace(
  /    function armPicker\(which\) \{[\s\S]*?\n    \}/,
  ''
);

// Remove old setRoutePref
newJs = newJs.replace(
...
// Remove onVehicleChange
newJs = newJs.replace(
  /    function onVehicleChange\(\) \{\n      activeVehicle = document\.getElementById\('vehicleType'\)\.value;\n    \}\n\n/,
  ''
);

// Remove calculateRoute
newJs = newJs.replace(
  /    async function calculateRoute\(\) \{[\s\S]*?\n    \}/,
  ''
);

// Remove fetchDestinationWeather
newJs = newJs.replace(
  /    async function fetchDestinationWeather\([^\)]*\) \{[\s\S]*?\n    \}/,
  ''
);
...
newJs = newJs.replace(
  /    function toggleSearchCollapse\(\) \{[\s\S]*?\n    \}/,
  ''
);

// Remove shareApp
newJs = newJs.replace(
  /    function shareApp\(\) \{[\s\S]*?\n    \}/,
  ''
);

// Update saveCurrentRoute
newJs = newJs.replace(
  /    function saveCurrentRoute\(\) \{[\s\S]*?\n    \}/,
...
// Remove old setupAutocomplete calls for fromInput/toInput if they still exist
newJs = newJs.replace(
  /setupAutocomplete\('fromInput', 'fromSuggest'\);/g,
  ''
);
newJs = newJs.replace(
  /setupAutocomplete\('toInput', 'toSuggest'\);/g,
  ''
);
