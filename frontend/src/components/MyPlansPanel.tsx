...
export const MyPlansPanel: React.FC<MyPlansPanelProps> = ({
  ...
  if (plans.length === 0) {
    return (
      <div className={`fixed top-20 right-4 z-[2000] w-72 rounded-2xl border shadow-2xl p-6 ${isDarkMode ? 'bg-slate-900/95 border-slate-700' : 'bg-white/95 border-white'
        }`}>
  ...
  return (
    <div className={`fixed top-20 right-4 z-[2000] w-72 md:w-80 max-h-[70vh] overflow-y-auto rounded-2xl border shadow-2xl ${isDarkMode ? 'bg-slate-900/95 border-slate-700' : 'bg-white/95 border-white'
      }`}>
      {/* Header */}
      ...
      <div className="p-4 space-y-3">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`rounded-xl border overflow-hidden transition-all ${isDarkMode
            ...
            >
              ...
              <button
                ...
                className={`w-full p-3 text-left transition-colors ${isDarkMode ? 'hover:bg-slate-700/50' : 'hover:bg-gray-100'
                  }`}
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className={`text-xs px-2 py-1 rounded-full ${plan.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400' :
              ...
              {/* Expanded Actions */}
              {isExpanded && (
                <div className={`p-3 border-t space-y-2 ${isDarkMode ? 'border-slate-700' : 'border-gray-200'
                  }`}>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-2 rounded-lg transition-colors text-sm ${isDarkMode
