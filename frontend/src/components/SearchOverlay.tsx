  ...
  return (
    <>
      <div className="absolute inset-0 z-[500] pointer-events-none">
        <div ref={cardRef} className="pointer-events-auto absolute top-20 left-4 right-4 z-[501]">
          {/* Search Input */}
          <div className="relative">
            <div className={`flex items-center gap-3 backdrop-blur-xl p-3 rounded-[1.5rem] shadow-[0_12px_32px_rgba(27,51,85,0.08)] border transition-colors duration-300 ${isDarkMode
          ...
          {/* Results Dropdown */}
          {showResults && (
            <div className={`mt-2 backdrop-blur-xl rounded-[1.5rem] shadow-[0_12px_32px_rgba(27,51,85,0.1)] border max-h-64 overflow-y-auto transition-colors duration-300 ${isDarkMode
              ...
              {results.length === 0 ? (
                ...
                results.map(r => (
                  <div
                    key={r.id}
                    className={`p-3 border-b last:border-b-0 cursor-pointer flex items-center justify-between group transition-colors duration-300 ${isDarkMode
                      ? 'border-slate-700/30 hover:bg-slate-700/50'
                      : 'border-outline/5 hover:bg-primary/5'
                      }`}
                    onClick={() => {
                      onSelectLocation(r);
