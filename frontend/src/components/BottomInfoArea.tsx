...
export const BottomInfoArea: React.FC<BottomInfoAreaProps> = ({ 
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[2000] p-4 flex justify-center pointer-events-none animate-in slide-in-from-bottom-20 duration-500">
      <div className="w-full max-w-4xl bg-white/80 backdrop-blur-xl border border-white/40 shadow-[0_-20px_50px_rgba(27,51,85,0.08)] rounded-t-[2.5rem] p-6 pointer-events-auto relative">
        
        {/* Close Button & TTS Toggle */}
        <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 flex items-center gap-2">
          <button 
            onClick={handleSpeak}
            className={`p-3 rounded-full border border-white/40 hover:scale-110 active:scale-95 transition-all shadow-xl ${isSpeaking ? 'bg-error text-white animate-pulse' : 'bg-white/80 backdrop-blur-md text-primary'}`}
          >
            <Volume2 size={20} />
          </button>
          <button 
            onClick={onClose}
            className="bg-white/80 backdrop-blur-md text-on-surface p-3 rounded-full border border-white/40 hover:scale-110 active:scale-95 transition-all shadow-xl"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-outline/10 flex items-center justify-between gap-4">
           <div className="flex-1 flex items-center gap-6 overflow-x-auto no-scrollbar pb-1">
              {[
                ...
                >
                   <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center text-primary group-hover:bg-gradient-to-br group-hover:from-primary group-hover:to-tertiary group-hover:text-white transition-all">
                      {s.icon}
                   </div>
                   <span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant group-hover:text-primary transition-all font-label">{s.label}</span>
                </button>
              ))}
           </div>
           
           <div className="hidden lg:block">
              <span className="text-[9px] font-bold text-outline/30 uppercase tracking-[0.4em] italic leading-none font-label">MEROSADAK SERVICES</span>
           </div>
        </div>
