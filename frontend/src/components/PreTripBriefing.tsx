...
export const PreTripBriefing: React.FC<PreTripBriefingProps> = ({ // isDarkMode is now from context
  briefing,
  onStartTrip,
  onAskAI,
  onClose,
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4">
      ...
      {/* Briefing Card */}
      <div className={`relative w-full max-w-md max-h-[80vh] overflow-y-auto rounded-2xl shadow-2xl ${isDarkMode ? 'bg-slate-900' : 'bg-white'
        }`}>
        {/* Header */}
        <div className={`sticky top-0 p-4 border-b ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'
          }`}>
        ...
        <div className="p-4 space-y-4">
          ...
          {briefing.weather.alerts && briefing.weather.alerts.length > 0 && (
            <div className={`p-3 rounded-xl border ${isDarkMode
              ...
              <div className="flex items-start gap-3">
                <CloudRain size={20} className={`mt-0.5 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'
                  }`} />
                <div>
                  ...
                  {briefing.weather.alerts.map((alert, idx) => (
                    <p key={idx} className={`text-xs mt-1 ${isDarkMode ? 'text-blue-400' : 'text-blue-600'
                      }`}>
                      ⚠️ {alert}
                    </p>
                  ))}
          ...
          {briefing.roadStatus.blockedSections > 0 && (
            <div className={`p-3 rounded-xl border ${isDarkMode
              ...
              <div className="flex items-start gap-3">
                <AlertTriangle size={20} className={`mt-0.5 ${isDarkMode ? 'text-red-400' : 'text-red-600'
                  }`} />
                <div>
                  <p className={`font-bold text-sm ${isDarkMode ? 'text-red-300' : 'text-red-700'
                    }`}>
                    Route Alert: {briefing.roadStatus.blockedSections} blocked section(s)
                  </p>
                  {briefing.roadStatus.changes?.map((change, idx) => (
                    <p key={idx} className={`text-xs mt-1 ${isDarkMode ? 'text-red-400' : 'text-red-600'
                      }`}>
                      • {change}
                    </p>
                  ))}
          ...
          {hasRecommendations && (
            <div className={`p-3 rounded-xl ${isDarkMode ? 'bg-slate-800/50' : 'bg-gray-50'
              ...
              <p className={`font-bold text-sm mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'
                }`}>
                💡 Recommendations
              </p>
              <ul className="space-y-1">
                {briefing.recommendations.map((rec, idx) => (
                  <li key={idx} className={`text-xs flex items-start gap-2 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
                    }`}>
          ...
          {/* Reminders */}
          {hasReminders && (
            <div className={`p-3 rounded-xl ${isDarkMode ? 'bg-amber-900/20' : 'bg-amber-50'
              }`}>
              <p className={`font-bold text-sm mb-2 ${isDarkMode ? 'text-amber-300' : 'text-amber-700'
                }`}>
                🔔 Reminders
              </p>
              <ul className="space-y-1">
                {briefing.reminders.map((rem, idx) => (
                  <li key={idx} className={`text-xs flex items-start gap-2 ${isDarkMode ? 'text-amber-400' : 'text-amber-600'
                    }`}>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              onClick={onAskAI}
              className={`px-4 py-3 rounded-xl font-bold transition-colors ${isDarkMode
